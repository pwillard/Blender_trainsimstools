...
def mapping_source_items(self, context):
    """Build a dropdown of existing image paths/names
    to use as the 'old' side of a mapping line."""
    items = []
    seen = set()
    ...
    for img in bpy.data.images:
        ...
        descr = path if path else img.name
        items.append((key, label, descr))

    if not items:
        items = [("__NONE__", "<no images found>", "No image filepaths available")]
...
class TXCH_OT_LoadMappingFromFile(Operator):
    ...
    def execute(self, context):
        ...
        if not p.mapping_file:
            self.report({'ERROR'}, "No mapping file specified.")
            return {'CANCELLED'}

        path = bpy.path.abspath(p.mapping_file)
        ...
        try:
            with open(path, "r", encoding="utf-8") as f:
                p.mapping_text = f.read()
        except Exception as e:
            self.report({'ERROR'}, f"Failed to read mapping file: {e}")
            return {'CANCELLED'}
...
class TXCH_OT_InsertMappingLine(Operator):
    ...
    bl_description = "Insert a 'old => ' line for the selected texture into the mapping"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        p = context.scene.txch
        key = p.mapping_choice
        ...
        if p.mapping_text.strip():
            p.mapping_text = p.mapping_text.rstrip() + "\n" + line
        else:
            p.mapping_text = line
...
class OBJECT_OT_CreateInitialCollections(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # MAIN_* under MAIN
        self.create_collection(context, "MAIN_300", parent_collection=main)
        self.create_collection(context, "MAIN_600", parent_collection=main)
        self.create_collection(context, "MAIN_1000", parent_collection=main)
        self.create_collection(context, "MAIN_1500", parent_collection=main)
...
# =============================
# Bounding Box CSV Export
# =============================

class TST_OT_ExportBoundingBoxCSV(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if blend_path:
            dirpath = os.path.dirname(blend_path)
        else:
            # Fallback for unsaved files: temp dir, then home as backup
            dirpath = bpy.app.tempdir or os.path.expanduser("~")

        filepath = os.path.join(dirpath, "bbox_export.csv")
        ...
        try:
            with open(filepath, "w", newline="", encoding="utf-8") as f:
                ...
                for obj in context.selected_objects:
                    ...
                    # Relative to cursor; -Y is forward
                    left_x_from_cursor   = min_x - cursor.x
                    right_x_from_cursor  = max_x - cursor.x
                    bottom_z_from_cursor = min_z - cursor.z
                    top_z_from_cursor    = max_z - cursor.z
                    rear_y_from_cursor   = max_y - cursor.y
                    front_neg_y_from_cursor = cursor.y - min_y

                    r = lambda v: round(v, 3)

                    writer.writerow([
                        obj.name,
                        r(left_x_from_cursor),
                        r(bottom_z_from_cursor),
                        r(rear_y_from_cursor),
                        r(right_x_from_cursor),
                        r(top_z_from_cursor),
                        r(front_neg_y_from_cursor),
                    ])
            ...
            self.report({'ERROR'}, f"Failed to export bounding boxes: {e}")
            return {'CANCELLED'}
...
class TXCH_PT_Panel(Panel):
    ...
    def draw(self, context):
        ...
        if p.strategy == 'SWAP_DIR':
            ...
            # Dropdown of existing textures
            col.label(text="Pick existing texture for 'old' side:")
            col.prop(p, "mapping_choice")
        ...
        # Bounding Box tools
        bbox_box = layout.box()
        bbox_box.label(text="Bounding Box Tools", icon='MESH_CUBE')
        bbox_box.label(text="Exports bbox_export.csv (3D Cursor origin, -Y forward).")
        bbox_box.operator("tst.export_bbox_csv", icon='FILE_TEXT')
...
class VIEW3D_PT_TrainSimToolsInfo(Panel):
    ...
    def draw(self, context):
        layout = self.layout
        layout.label(text="TrainSimTools v1.3.10")
        layout.label(text="Combined utilities for texture + collections")
        layout.label(text="© Pete Willard")
        layout.operator('wm.url_open', text='Visit Documentation', icon='HELP').url = 'https://github.com/pwillard/Blender_trainsimstools'
