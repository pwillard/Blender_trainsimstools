...
- Mapping strategy now has a file chooser for loading mapping rules from a text file.
...
def mapping_source_items(self, context):
    """
    Build a dropdown of existing image paths/names to use as the 'old' side
    of a mapping line.
    """
    items = []
    seen = set()
    ...
    if not items:
        items = [
            ("__NONE__", "<no images found>", "No image filepaths available")
        ]
...
class VIEW3D_PT_TrainSimToolsInfo(Panel):
    ...
    def draw(self, context):
        layout = self.layout
        layout.label(text="TrainSimTools v1.3.9")
